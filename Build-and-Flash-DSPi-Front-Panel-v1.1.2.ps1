...
function Ensure-Git {
    ...
    if (Get-Command winget.exe -ErrorAction SilentlyContinue) {
        ...
        & winget.exe install --id Git.Git --exact --accept-package-agreements --accept-source-agreements
        if ($LASTEXITCODE -ne 0) { throw "Git installation failed." }
        $env:Path = ([Environment]::GetEnvironmentVariable("Path", "Machine") + ";" + [Environment]::GetEnvironmentVariable("Path", "User"))
    }
    if (-not (Get-Command git.exe -ErrorAction SilentlyContinue)) {
        throw "Git is required to prepare the v1.1.2 source. Install Git for Windows and rerun this script."
    }
}

function Prepare-ReleaseSource {
    if (-not (Test-Path -LiteralPath $PatchFile -PathType Leaf)) {
        throw "Release source update not found: $PatchFile"
    }
    Ensure-Git
    Push-Location $Root
    try {
        & git.exe apply --check --ignore-space-change --ignore-whitespace $PatchFile 2>$null
        if ($LASTEXITCODE -eq 0) {
            Write-Host "Applying DSPi ESP32 Front Panel v1.1.2 source update..." -ForegroundColor Cyan
            & git.exe apply --ignore-space-change --ignore-whitespace $PatchFile
            if ($LASTEXITCODE -ne 0) { throw "The v1.1.2 source update could not be applied." }
            return
        }
        & git.exe apply --reverse --check --ignore-space-change --ignore-whitespace $PatchFile 2>$null
        if ($LASTEXITCODE -eq 0) {
            Write-Host "DSPi ESP32 Front Panel v1.1.2 source is already prepared." -ForegroundColor Green
            return
        }
        throw "The source tree does not match the expected v1.1.2 base. Download a fresh copy of this branch."
    }
    finally { Pop-Location }
}

Prepare-ReleaseSource
...
if (-not $FlashOnly) {
    ...
    $checksumRows | Set-Content -LiteralPath (Join-Path $ReleaseDir "SHA256SUMS.txt") -Encoding ASCII
}
